...
(* Allocation: lists (MAKEBLOCK2), tuples (MAKEBLOCK2/3, MAKEBLOCK),
   closures with an environment (CLOSURE), and reading them back. *)
...
let () =
  let l = build 10 [] in
  let s = sum l in                         (* 55 *)
  let s2 = sum (map (adder 100) l) in      (* 1055 *)
  let (a, b) = pair s s2 in
  let (c, d, e) = triple 1 2 3 in
  let (f, g, h, i) = quad 4 5 6 7 in
  print_int (a + b + c + d + e + f + g + h + i);   (* 1138 *)
  output_char stdout '\n'; flush stdout
