...
let rx_len () = io_read eth_rxlen land 0x7FF
let rx_ready () = io_read eth_status land eth_rx_valid <> 0
let rx_done () = io_write eth_rxlen 0
let eth_send len =
  while io_read eth_status land eth_tx_busy <> 0 do () done;
  let len = if len < 60 then begin for i = len to 59 do tx i 0 done; 60 end else len in
  io_write eth_txlen len
let uart_putc c = io_write uart (int_of_char c)
let uart_getc () = io_read uart_rx
let set_leds v = io_write leds v
let phy () = io_read eth_status_phy
(* ==== END HW ==== *)
...
let () =
  set_leds 1;
  uart_puts "telnet (OCaml processor): phy=";
  uart_hex16 (phy ());
  uart_puts "\nlistening on TCP 23 once DHCP has an address\n";
